...
public class CrearPaciente extends CrearModificarPaciente {

    public CrearPaciente(PanelManager m) {
        super(m);
    }
    ...
    protected void ejecutar() {
        ...
        if (dni > 0 && nombre.length() > 0 && apellido.length() > 0 && ano > 1900 && ano < 2021 && mes > 0 && mes < 13
            ...
            try {
                String respuesta = PacienteService.crearPaciente(dni, nombre, apellido, fecha_nacimiento, obraSocial);
                int respuesta_joption = JOptionPane.showConfirmDialog(this, respuesta,
                        "Creando Paciente", JOptionPane.OK_CANCEL_OPTION);
                if (respuesta_joption == JOptionPane.OK_OPTION) {
                    m.mostrarPantallaPaciente();
                }
            } catch (ServicioException e) {
                JOptionPane.showMessageDialog(this, "Hubo un problema al crear el paciente",
