...
public class ModificarMedico extends CrearModificarMedico {

    public ModificarMedico(PanelManager m) {
        super(m);
    }
    ...
    protected void ejecutar() {
        ...
        if (dni > 0 && nombre.length() > 0 && apellido.length() > 0 && ano > 1900 && ano < 2021 && mes > 0 && mes < 13
            ...
            try {
                String respuesta = MedicoService.modificarMedico(dni, nombre, apellido, fecha_nacimiento, montoConsulta, listadoObraSocial);
                int respuesta_joption = JOptionPane.showConfirmDialog(this, respuesta,
                        "Modificando Medico", JOptionPane.OK_CANCEL_OPTION);
                if (respuesta_joption == JOptionPane.OK_OPTION) {
                    m.mostrarPantallaMedicos();
                }
            } catch (ServicioException e) {
                JOptionPane.showMessageDialog(this, "Hubo un problema al modificar el medico",
