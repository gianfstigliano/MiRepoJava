                ...
                try {
                    if (dni_medico_txt.length() != 0) {
                        if (ano.length() > 0 && mes.length() > 0 && dia.length() > 0) {
                            ...
                            listado = TurnoService.consultarTurnoMecioFecha(dni_medico_txt, fecha_turno_String);
                            m.mostrarPantallaLISTARTurno(listado);
                        } else {
                            listado = TurnoService.consultarTurnoMedico(dni_medico_txt);
                            m.mostrarPantallaLISTARTurno(listado);
                        }
                    } else if (dni_paciente_txt.length() != 0) {
                        listado = TurnoService.consultarTurnoPaciente(dni_paciente_txt);
                        m.mostrarPantallaLISTARTurno(listado);
                    } else {
